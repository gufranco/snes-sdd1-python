...
class TruncatedStream(Exception):
    """The stream ran out before it had produced what its header promised.

    Raised rather than returning what was decoded so far, because a short answer
    that looks like a whole one is the failure this decompressor is easiest to
    get wrong in: a caller cannot tell a truncated block from a small one.
    """
