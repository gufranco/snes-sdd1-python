...
class BitplaneTest(unittest.TestCase):
    def test_each_type_names_how_many_planes_it_interleaves(self):
        self.assertEqual(
...
class HeaderTest(unittest.TestCase):
    def test_a_stream_shorter_than_its_header_is_refused(self):
    ...
    def test_a_stream_that_runs_out_mid_decode_is_refused(self):
        with self.assertRaises(decoder.TruncatedStream):
            decoder.decompress(bytes([0xFF, 0xFF]), 0, 0x1000)

    def test_the_header_decides_how_many_planes_are_reported(self):
        found = decoder.decompress(bytes([0xC0]) + BLOB[:64], 0, 8)

        self.assertEqual(found.bitplanes, 8)

    def test_the_header_decides_which_context_masks_are_used(self):
...
class LengthTest(unittest.TestCase):
    def test_a_stream_gives_back_the_length_asked_for(self):
        for length in (1, 2, 3, 15, 16, 17, 64, 833):
            found = decoder.decompress(BLOB, 0, length)

            self.assertEqual(len(found.data), length)

    def test_a_length_of_zero_means_one_whole_block(self):
        found = decoder.decompress(BLOB, 0, 0)

        self.assertEqual(len(found.data), decoder.MAX_LENGTH)

    def test_a_stream_reports_where_it_stopped_reading(self):
        found = decoder.decompress(BLOB, 0, 64)

        self.assertGreater(found.end, 2)
        self.assertLessEqual(found.end, len(BLOB))

    def test_a_longer_read_consumes_at_least_as_much_stream(self):
...
class EveryHeaderTest(unittest.TestCase):
    def test_every_combination_of_plane_and_context_type_decodes(self):
    ...
    def test_every_plane_type_reports_its_own_plane_count(self):
        for bitplane_type in range(4):
