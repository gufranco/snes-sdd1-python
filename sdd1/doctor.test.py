...
def a_corpus(cases=3, reference="an independent encoder"):
...
class FindingTest(unittest.TestCase):
    def test_a_finding_says_what_was_checked(self):
        self.assertEqual(a_finding(name="the vectors").name, "the vectors")
    ...
    def test_every_finding_carries_what_it_actually_saw(self):
        self.assertIn("512 cases", a_finding(detail="512 cases").line)
...
class ExamineTest(unittest.TestCase):
    ...
    def test_and_the_version_of_this_package(self):
        self.assertIn("snes-sdd1-python", [one.name for one in doctor.examine()])

    def test_the_package_and_the_part_do_not_share_a_name(self):
...
class DecoderTest(unittest.TestCase):
    """That the decoder is exercised here rather than assumed to work."""

    def test_the_report_says_what_the_decoder_did(self):
        self.assertIn("decoder", [one.name for one in doctor.examine()])

    def test_a_decoder_that_refuses_an_empty_stream_is_well(self):
        for one in doctor.examine():
            if one.name == "decoder":
                self.assertTrue(one.ok)
                self.assertIn("refuses", one.detail)

    def test_one_that_invents_bytes_instead_is_a_failure(self):
        found = doctor._decoder(lambda _rom, _offset, length: b"\x00" * length)

        self.assertFalse(found.ok)
        self.assertIn("8 bytes", found.detail)

    def test_and_one_that_throws_something_else_is_reported_as_what_it_threw(self):
        def boom(_rom, _offset, _length):
            raise Complaint("nothing decodes")

        found = doctor._decoder(boom)
...
class CorpusTest(unittest.TestCase):
    def test_every_corpus_is_reported_under_its_own_name(self):
        names = [one.name for one in doctor.examine()]

        for label, _ in doctor.CORPORA:
            self.assertIn(label, names, label)

    def test_a_corpus_that_is_here_is_counted(self):
        found = doctor.examine(corpora=(("made up", a_corpus(cases=7)),))
    ...
    def test_and_carries_the_digest_of_the_file_that_is_actually_here(self):
        ...
        found = doctor.examine(corpora=(("made up", where),))
    ...
    def test_and_names_what_it_was_recorded_from(self):
        found = doctor.examine(corpora=(("made up", a_corpus(reference="somebody else 2.0")),))
    ...
    def test_a_corpus_that_is_not_here_is_a_failure(self):
        found = doctor.examine(corpora=(("made up", Path("/nowhere/at/all.json")),))

        self.assertTrue(any(one.name == "made up" and not one.ok for one in found))
    ...
    def test_the_corpora_it_reads_by_default_are_the_ones_in_this_repository(self):
        for _, where in doctor.CORPORA:
...
class ReportTest(unittest.TestCase):
    ...
    def test_it_opens_with_something_that_says_what_it_is(self):
        self.assertIn("sdd1", doctor.report(doctor.examine())[0])
