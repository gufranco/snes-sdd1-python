...
def a_corpus(cases: int = 3, reference: str = "an independent encoder") -> Path:
...
class DecoderTest(unittest.TestCase):
    ...
    def test_the_report_says_what_the_decoder_did(self) -> None:
        self.assertIn("decoder", [one.name for one in doctor.examine()])

    def test_a_decoder_that_refuses_an_empty_stream_is_well(self) -> None:
        for one in doctor.examine():
            if one.name == "decoder":
                self.assertTrue(one.ok)
                self.assertIn("refuses", one.detail)

    def test_one_that_invents_bytes_instead_is_a_failure(self) -> None:
    ...
    def test_and_one_that_throws_something_else_is_reported_as_what_it_threw(self) -> None:
        def boom(_rom: Any, _offset: int, _length: int) -> Any:
...
class CorpusTest(unittest.TestCase):
    def test_every_corpus_is_reported_under_its_own_name(self) -> None:
        names = [one.name for one in doctor.examine()]
    ...
    def test_the_corpora_it_reads_by_default_are_the_ones_in_this_repository(self) -> None:
