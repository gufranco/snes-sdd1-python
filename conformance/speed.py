...
Not a benchmark for its own sake. The two corpora hold 6,652 streams between
them and every one of them is expanded on every run. The way that stops being
usable is gradual: a table read grows an allocation, a context lookup becomes a
comprehension, and a year later a run nobody changed takes an hour. A floor that
fails loudly is cheaper than noticing.
...
STREAM = bytes(4096)
"""A stream of zeroes, long enough that the decoder never runs out of input.

Zeroes rather than noise, because a floor should measure the decoder rather than
how lucky one particular stream is with the probability model.
"""
...
def measure(calls: int = CALLS, repeats: int = REPEATS) -> Timed:
    ...
    part = sdd1.Chip(MODEL).reset()
    seconds = []
    for _ in range(repeats):
        started = time.perf_counter()
        part.decompress(STREAM, 0, calls)
