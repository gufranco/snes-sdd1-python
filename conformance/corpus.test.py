...
class DefinitionTest(unittest.TestCase):
    ...
    def test_the_corpus_names_the_decoder_its_answers_came_from(self) -> None:
        self.assertIn("snes9x", corpus.load()["reference"])

    def test_the_corpus_records_the_census_it_was_shaped_by(self) -> None:
...
class BodyTest(unittest.TestCase):
    def test_the_image_is_rebuilt_from_its_seed(self) -> None:
        found = corpus.load()

        self.assertEqual(len(corpus.body(found)), found["body_bytes"])

    def test_the_same_seed_rebuilds_the_same_image(self) -> None:
        found = corpus.load()

        self.assertEqual(corpus.body(found), corpus.body(found))

    def test_every_case_has_its_real_header_at_its_offset(self) -> None:
        found = corpus.load()
...
class ShapeTest(unittest.TestCase):
    def test_every_header_the_cartridges_use_appears(self) -> None:
        found = corpus.load()

        self.assertGreater(len({case["header"] for case in found["cases"]}), 60)

    def test_the_cases_reach_a_whole_block(self) -> None:
        found = corpus.load()

        self.assertIn(0x10000, {case["length"] for case in found["cases"]})

    def test_the_cases_reach_short_streams_as_well(self) -> None:
        found = corpus.load()

        self.assertLess(min(case["length"] for case in found["cases"]), 16)

    def test_the_corpus_holds_no_compressed_byte_of_a_cartridge(self) -> None:
        found = corpus.load()
...
class CheckTest(unittest.TestCase):
    ...
    def test_the_image_is_shared_by_every_case(self) -> None:
        found = corpus.load()

        self.assertGreater(len(found["cases"]), 1)

    def test_a_disagreement_names_the_header_and_length(self) -> None:
        found = corpus.load()
...
class MainTest(unittest.TestCase):
    ...
    def test_a_disagreeing_corpus_fails_and_names_the_case(self) -> None:
        found = corpus.load()
        broken = dict(found, cases=[dict(found["cases"][0], output_sha256="0" * 64)])
