...
class ScanTest(unittest.TestCase):
    ...
    def test_a_rom_with_no_tags_yields_nothing(self):
        self.assertEqual(extract.tagged_streams(a_rom([])), [])

    def test_a_tag_at_the_very_end_is_not_a_stream(self):
        rom = bytearray(a_rom([]))
...
class ShapeTest(unittest.TestCase):
    def test_a_header_is_read_at_each_offset(self):
    ...
    def test_no_stream_byte_is_carried_into_the_shapes(self):
...
class MainTest(unittest.TestCase):
    ...
    def test_a_rom_that_is_not_there_is_reported(self):
        code, output = self.run_main([str(Path(self.root) / "absent.sfc"), "out.json"])

        self.assertEqual(code, 2)
        self.assertIn("no rom at", output)

    def test_a_tagged_rom_is_turned_into_a_shape_profile(self):
    ...
    def test_a_rom_with_no_streams_says_so(self):
    ...
    def test_a_table_can_be_given_instead_of_scanning(self):
